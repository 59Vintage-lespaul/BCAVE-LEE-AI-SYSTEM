...
## 사용법
`/value-prop <대상 상품 또는 브랜드>` — 대상을 생략하면 리 코어 타겟 기준으로 작성합니다.

## 절차

### Step 1: 컨텍스트 파악
1. `.fpof-state.json`에서 현재 시즌과 단계 확인
2. `system/presets/lee/personas.json` 참조 — 페르소나, JTBD
3. `system/presets/lee/brand.config.json` 참조 — 포지셔닝, 차별화
4. `system/presets/lee/tone-manner.json` 참조 — 브랜드 보이스
5. 사용자에게 확인:
   - 대상 상품/브랜드
