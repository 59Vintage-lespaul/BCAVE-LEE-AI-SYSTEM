...
## 사용법
`/strategy-canvas <대상 브랜드 또는 상품>` — 대상을 생략하면 리 전체 브랜드 전략을 수립합니다.

## 절차

### Step 1: 컨텍스트 파악
1. `.fpof-state.json`에서 현재 시즌과 단계 확인
2. `system/presets/lee/brand.config.json` 참조 — DNA, 비전, 포지셔닝
3. `system/presets/lee/personas.json` 참조 — 타겟 세그먼트
4. `system/presets/lee/categories.json` 참조 — 카테고리 전략
5. 사용자에게 확인:
