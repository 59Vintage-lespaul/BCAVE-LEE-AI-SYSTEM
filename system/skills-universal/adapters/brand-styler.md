# Brand Styler — 리 브랜드 적용 가이드

> **어댑터 출처:** `.claude/skills/brand-guidelines/SKILL.md` 기반, Anthropic 브랜드 참조 제거
> **대상:** 리(Lee) 브랜드 전용 스타일 적용
> **원본 브랜드 소스:** `presets/lee/visual-identity.json`
...
**디자인 키워드:** Doodle · Graffiti · Workwear Heritage · Bold Lines · Vivid Colors
...
- Buddy Lee(Kiki)와 11 캐릭터 활용 시 `presets/lee/ip-bible.json` 참조
...
- 이 가이드는 **리 전용**입니다. 다른 브랜드 출력물에는 사용하지 마십시오.
- 시즌별 Secondary 컬러는 `presets/lee/visual-identity.json` 최신 값 확인
- 고객 대면 콘텐츠는 반드시 `presets/lee/tone-manner.json`도 함께 참조
- IP 캐릭터(Buddy Lee, 11 캐릭터) 활용 시 `presets/lee/ip-bible.json` 참조
