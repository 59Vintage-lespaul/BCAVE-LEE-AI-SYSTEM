...
> `presets/lee/visual-identity.json`을 읽기 전용으로 참조하여 브랜드 컬러를 자동 적용합니다.
...
```bash
  ...
  --preset-dir "presets/lee" # 브랜드 폴더 (기본: 자동 탐색)
  --quiet                            # 진행 메시지 숨기기
```
...
시스템은 `presets/lee/visual-identity.json`에서 아래 컬러를 자동 로드합니다:
...
**브랜드 컬러 미적용**: `presets/lee/visual-identity.json` 경로 확인 또는 `--preset-dir` 지정
