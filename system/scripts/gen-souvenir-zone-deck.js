...
function addFooter(slide, pageNum, totalPages) {
  ...
  slide.addText("Lee | Confidential", {
...
async function main() {
  ...
  const summaryPoints = [
    ...
    ["핵심 콘셉트", "상설 Buddy Lee 구조물 랜드마크 + 격월 테마 교체 + 커스텀 프린트 + 한정 굿즈"],
  ...
  const targets = [
    ...
    [icons.heart_d, "코어 타겟 18~29세", "40~50%", "한정판 굿즈 수집\n캐릭터 팬덤\n나노 커뮤니티 이벤트"],
  ...
  s.addText("\"하나의 랜드마크, 변하는 무대\" — Buddy Lee 상설 구조물 1기 + 주변 소품/배경 격월 교체", {
    x: 0.7, y: 1.15, w: 8.6, h: 0.45, fontSize: 12, fontFace: "Arial", color: C.MINT, margin: 0, valign: "middle"
  });

  // 3 zones
  const zones = [
    [icons.camera_d, "Zone A", "Buddy Lee 구조물 + 포토존", "~3평", "상설 Buddy Lee FRP 2.5m\n테마별 소품/배경 교체\n시선 스토퍼 + 랜드마크", C.TEAL],
  ...
  const themes = [
    ["T1", "3~4월", "Grand Opening\nWacky Universe", "Buddy Lee + 전체", "자체 브랜딩", C.TEAL],
    ["T2", "5~6월", "Summer\nPlayground", "쿼바오 + Buddy Lee", "테라 맥주", "68A8DB"],
  ...
  const t1Items = [
    "주인공: Buddy Lee + 전체 12캐릭터 총출동",
    "구조물: Buddy Lee 조형물 + 12캐릭터 소개 배너 + 옐로 풍선 아치",
    "포토존: \"Buddy Lee와 같은 포즈\" 바닥 가이드 스티커",
    "커스텀: 12캐릭터 전체 셀렉 + SEONGSU 2026 배경",
    "한정 굿즈: 오프닝 키링 세트(3종), 성수 한정 그래픽티",
    "이벤트: 오프닝 1주차 커스텀 프린트 50% 할인",
    "인플루언서: 이사배 × Buddy Lee \"변신 메이크업\" 콜라보",
  ...
  const t3Items = [
    ...
    "포토존: 플라워로 둘러싸인 Buddy Lee와 함께 \"정원 파티\" 촬영",
  ...
  const t5Items = [
    ...
    "포토존: 크리스마스 분위기 Buddy Lee, 산타 모자/루돌프 머리띠 대여",
  ...
  const t6Items = [
    ...
    "포토존: Buddy Lee와 나란히 러닝 포즈, 넘버링 빕 대여",
  ...
  const priceHeader = [
    ...
    [
      ...
      { text: "리 브랜딩 박스 + 리본", options: { fontSize: 10, fontFace: "Arial", color: C.GRAY_600 } },
    ],
  ];
  ...
  const collabHeader = [
    [
      ...
      { text: "리 연결", options: { fill: { color: C.BLACK }, color: C.WHITE, bold: true, fontSize: 10, fontFace: "Arial" } },
  ...
  const collabRows = [
    ...
    ["서울 해치", "도시 캐릭터", "Denim Heritage + 관광객", "T3", "★★☆"],
  ...
  const influencers = [
    ["이사배", "Buddy Lee (변신)", "변신 키트, 오프닝 프린트", "T1", "일·중·동남아 ★★★", "Tier 2"],
  ...
  const setupItems = [
    ["Buddy Lee 상설 구조물", "500~800만", C.TEAL],
