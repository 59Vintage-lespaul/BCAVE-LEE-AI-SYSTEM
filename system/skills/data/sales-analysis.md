...
## 사전 준비
1. `presets/lee/channels.json` → 채널별 목표 매출
2. `presets/lee/brand.config.json` → 5대 경영목표 KPI 기준
3. `presets/lee/personas.json` → 고객 세그먼트 기준
...
리의 비즈니스 게임을 분류하고 North Star Metric을 정의한다.
...
→ 리는 **Transaction Game** (패션 커머스) + **Attention Game** (IP 기반 팬덤) 하이브리드
