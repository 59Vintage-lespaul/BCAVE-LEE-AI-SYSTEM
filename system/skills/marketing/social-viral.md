...
2. `presets/lee/personas.json` → 타겟의 미디어 소비, 팔로우 인플루언서 유형
3. `presets/lee/channels.json` → 온라인 채널 전략
...
- 브랜드핏: 리 감성(SENSUOUS WORKWEAR, 헤리티지 데님, 감각적 재해석)과의 정합성
...
### 5가지 그로스 루프 유형 평가
리 맥락에서 각 루프의 적합성을 평가한다:

| # | 루프 유형 | 메커니즘 | 리 적용 예시 | 적합도 (1-10) |
|---|---------|---------|-----------------|-------------|
| 1 | **Viral Loop** | 콘텐츠 생성 → 외부 공유 → 신규 유입 | 착용샷/스타일링 공유 → 인스타/TikTok 노출 → 브랜드 발견 | |
| 2 | **Usage Loop** | 사용 → 콘텐츠 생성 → 공유 → 재참여 | Buddy Lee 캐릭터 커스텀 → 공유 → 타인 참여 | |
| 3 | **Collaboration Loop** | 초대 → 공동 참여 → 가치 발견 | 친구 태그 이벤트 → 함께 쇼핑 → 공동 구매 | |
| 4 | **UGC Loop** | 발견 → 자체 콘텐츠 생성 → 공유 → 타인 발견 | 리 OOTD → TikTok 트렌드 → 모방 콘텐츠 | |
