...
2. `presets/lee/visual-identity.json` → 비주얼 스타일 기준
3. `presets/lee/channels.json` → 채널 구조 및 우선순위
4. `presets/lee/tone-manner.json` → 카피 톤 기준
5. `presets/lee/personas.json` → 모델/타겟 체형 참고
6. `presets/lee/ip-bible.json` → 캐릭터 활용 시
...
[Brand]: 리 비주얼 키워드 (Doodle, Graffiti, Workwear Heritage, Bold Lines, Vivid Colors)
