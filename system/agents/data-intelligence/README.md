...
- **필수 참조 파일**:
  - `presets/lee/brand.config.json` (Relevance 스코어링 기준)
  - `presets/lee/categories.json` (BTA 매핑, 카테고리 연결)
  - `presets/lee/personas.json` (코어타겟 부합도)
  - `presets/lee/visual-identity.json` (비주얼 시그널 필터)
...
- **필수 참조 파일**:
  ...
  - `presets/lee/categories.json` (BTA 전략)
  - `presets/lee/brand.config.json` (경영목표 #3 QR 60%)
...
- **필수 참조 파일**:
  - `presets/lee/channels.json` (채널별 목표 대비 실적 비교)
  - `presets/lee/brand.config.json` (경영목표 KPI 기준)
  - `presets/lee/personas.json` (고객 데이터 인사이트 비교)
...
- **필수 참조 파일**:
  ...
  - `presets/lee/brand.config.json` (경영목표와 연결)
