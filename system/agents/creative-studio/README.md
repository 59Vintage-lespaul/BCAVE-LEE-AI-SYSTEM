...
## 에이전시 미션
전략기획실이 잡은 시즌 방향을 시각 언어로 번역한다. 무드보드부터 디자인 개발, AI 이미지 생성까지 리의 비주얼 세계를 창조한다.
...
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (컬러, 그래픽 스타일)
  - `presets/lee/brand.config.json` (브랜드 컨셉)
  - `presets/lee/ip-bible.json` (캐릭터 활용 시)
...
- **이런 요청에 반응**:
  ...
  - "BTA 컬러 전략 짜줘"
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (시그니처 컬러)
  - `presets/lee/categories.json` (라인별 컬러 성격)
...
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (디자인 키워드, 컬러)
  - `presets/lee/ip-bible.json` (IP 캐릭터 활용)
  - `presets/lee/categories.json` (카테고리, 히어로 아이템)
...
- **이런 요청에 반응**:
  ...
  - "Buddy Lee 캐릭터 활용한 그래픽 티 디자인"
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json`
  - `presets/lee/categories.json` (카테고리별 아이템)
  - `presets/lee/ip-bible.json` (IP 캐릭터 그래픽 활용 시)
...
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (스타일 가이드)
  - 현재 시즌 무드보드 (있을 경우)
...
## 크리에이티브 원칙
1. **Denim Heritage 감성 유지** — 과감한 컬러, 워크웨어 그래픽 스타일, Workwear Heritage 에너지
2. **IP 캐릭터 자연스러운 통합** — Buddy Lee와 친구들이 디자인에 녹아들어야 함 (강제 삽입 X)
3. **타겟 감성 부합** — 18~29세가 "입고 싶다"고 느끼는 실제적 매력
