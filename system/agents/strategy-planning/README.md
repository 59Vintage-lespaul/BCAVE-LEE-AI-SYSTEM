...
## 에이전시 미션
리의 시즌 전략을 수립하고, 시장과 고객을 분석하여 브랜드가 올바른 방향으로 나아가도록 나침반 역할을 한다.
...
- **이런 요청에 반응**:
  ...
  - "리 정체성이 뭐야?", "경쟁사 대비 우리 위치는?"
- **필수 참조 파일**:
  - `presets/lee/brand.config.json` (브랜드 DNA, 비전, 전략 방향)
...
- **필수 참조 파일**:
  - `presets/lee/categories.json` (카테고리 트리, 상품 전략)
...
- **이런 요청에 반응**:
  ...
  - "글로벌 트렌드 리포트 만들어줘"
- **필수 참조 파일**:
  - `presets/lee/brand.config.json` (전략 방향과 정합 확인)
