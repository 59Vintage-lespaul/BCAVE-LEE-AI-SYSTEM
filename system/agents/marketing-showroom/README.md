...
## 에이전시 미션
리의 상품과 세계관을 고객에게 전달하는 모든 커뮤니케이션을 책임진다. IMC 전략 수립부터 화보/영상 콘텐츠 제작, 스토리텔링, 인플루언서/바이럴까지 마케팅의 풀 스펙트럼을 커버한다.
...
- **이런 요청에 반응**:
  ...
  - "이번 시즌 캠페인 큰 그림 그려줘"
- **필수 참조 파일**:
  - `presets/lee/channels.json` (채널별 매출/목표)
  - `presets/lee/brand.config.json` (경영목표, 전략 방향)
  - `presets/lee/personas.json` (타겟 고객)
- **관장 범위**:
...
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (비주얼 스타일 기준)
  - `presets/lee/channels.json` (채널 규격 및 우선순위)
  - `presets/lee/tone-manner.json` (카피 톤 기준)
...
- **필수 참조 파일**:
  - `presets/lee/visual-identity.json` (비주얼 스타일 가이드)
  - `presets/lee/tone-manner.json` (브랜드 톤)
  - `presets/lee/ip-bible.json` (캐릭터 활용 시)
- **크리에이티브 스튜디오와의 협업**:
...
- **필수 참조 파일**:
  - `presets/lee/tone-manner.json` (톤앤매너 — 반드시 준수)
  - `presets/lee/ip-bible.json` (캐릭터 스토리 연결 시)
  - `presets/lee/personas.json` (타겟 언어/감성)
...
- **필수 참조 파일**:
  - `presets/lee/personas.json` (타겟 고객의 미디어 소비 습관)
  - `presets/lee/channels.json` (온라인 채널 전략)
