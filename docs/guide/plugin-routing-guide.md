...
| **특화 영역** | 리 브랜드, 패션 산업, PDCA 사이클 | 범용 비즈니스 (HR, 법무, 재무, 엔지니어링, 영업 등) |
...
| **브랜드 톤** | "리 톤으로 써줘" (tone-manner.json) | "@brand 새 브랜드 가이드라인 생성" |
| **대시보드** | `/sheet` `/metrics` (리 KPI) | "@data 인터랙티브 HTML 대시보드" |
...
"매출 분석해줘"               → FPOF sales-analysis (리 맥락 자동 감지)
...
| IMC/바이럴 | imc-strategy, social-viral, visual-content | 마케팅 쇼룸 (리 특화) |
| 품질/QC | quality-gate, gap-analysis, completion-report, pdca-iteration | QC 본부 |
| 인사이트 | sales-analysis, insight-archiving | 리 매출 데이터 기반 |
...
AI: → FPOF competitive-battlecard (리 맥락 유지)
```
...
```
사용자: "캠페인 기획해줘"
AI: (현재 PDCA Do 단계 + 리 맥락) → FPOF imc-strategy
...
```
사용자: "이번 주 매출 분석해줘"
AI: → FPOF sales-analysis (리 매출 데이터, weekly/data/ 참조)
...
**A:** KW 플러그인 결과물 중 리 프로젝트에 포함되는 산출물은 FPOF 저장 규칙(`workspace/` 구조, 파일명 규칙)을 따릅니다. 그 외 범용 결과물(코드 리뷰, 법률 문서 등)은 별도 경로에 저장합니다.
